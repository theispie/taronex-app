...
## ⏸ ที่เก็บไฟล์ภายนอก — รอจดโดเมนจริง

**ตัดสิน 25 ส.ค. 2569** — รอจดโดเมนจริงก่อนแล้วค่อยต่อ Cloudflare R2 ทีเดียว
เหตุผล: CORS ของ R2 ต้องระบุโดเมนที่เบราว์เซอร์ยิงมา ถ้าต่อตอนนี้ด้วยโดเมนชั่วคราว
แล้วเปลี่ยนโดเมนทีหลัง ต้องกลับมาแก้ CORS อยู่ดี

เตรียมสคริปต์ไว้ให้รันได้ทันทีในวันนั้นแล้ว — สร้างถัง ตั้ง CORS
ทดสอบเขียนอ่านลบครบรอบ แล้วเขียนลงไฟล์ตั้งค่าให้เอง
...
ต้องมีสามค่าจาก Cloudflare → R2 → Manage API Tokens (สิทธิ์ Object Read & Write)
Account ID · Access Key ID · Secret Access Key

### ระหว่างที่ยังไม่ต่อ — อะไรใช้ไม่ได้บ้าง
| | สถานะ |
|---|---|
| ไฟล์แนบในการ์ด (หน้า 37) · ไฟล์แนบจากพอร์ทัล | **ใช้ไม่ได้** — presigned URL ชี้ไป MinIO บน 127.0.0.1 ที่เบราว์เซอร์เข้าไม่ถึง |
| รูปโปรไฟล์ | ใช้ได้ — เก็บบนดิสก์เครื่อง แอปเสิร์ฟเอง ไม่ผ่านที่เก็บไฟล์ |
| สำรองฐานข้อมูล | ใช้ได้ แต่**อยู่บนดิสก์ก้อนเดียวกับฐาน** ไม่มีสำเนานอกเครื่อง |

หน้า `/meta/health` รายงานตรงๆ ว่า *"ต่อติดแต่ใช้จริงไม่ได้ · MinIO บนเครื่อง (เบราว์เซอร์เข้าไม่ถึง)"*
ไม่ขึ้นแค่ `ok` เพราะจะทำให้นึกว่าไฟล์แนบพร้อมใช้

### ⚠ ระหว่างนี้ ความเสี่ยงที่เหลืออยู่คือ droplet หาย
สำรองบนดิสก์กันได้แค่ลบผิด · migration พัง · ข้อมูลเสีย
ถ้าอยากกัน droplet หายโดยไม่ต้องรอ R2 ทางที่เร็วที่สุดคือเปิด
**DigitalOcean Backups** (สแนปช็อตอัตโนมัติรายสัปดาห์) ในแดชบอร์ด DO ที่ใช้อยู่แล้ว
